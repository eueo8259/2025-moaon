...
/**
 * Sorting policy for project love count based pagination.
 */
@Component
public class LovesProjectSortSpec implements ProjectSortSpec {

    @Override
    public ProjectSortType getType() {
        return ProjectSortType.LOVES;
    }
    ...
    public BooleanExpression getCursorPredicate(CursorToken token) {
        ...
        Integer loves = parseSortValue(token);
        return project.lovedMembers.size().lt(loves)
                .or(project.lovedMembers.size().eq(loves).and(project.id.lt(token.lastId())));
    }

    @Override
    public String createNextCursor(Project project) {
        return CursorCodec.encode(new CursorToken(String.valueOf(project.getLoveCount()), project.getId()));
    }
